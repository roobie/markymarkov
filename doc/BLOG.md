...
Marky extracts parent-child transitions from this tree:
```
(Module, FunctionDef)
(FunctionDef, If)
(FunctionDef, For)
...
Marky detects and tracks 52+ distinct semantic patterns across these categories:
...
The magic of Marky comes from combining these two levels:
...
*Note: This chapter explores integration patterns between Marky and LLM agents. While not yet implemented in practice, the architecture suggests several natural approaches worth considering.*

Marky's design lends itself to integration with LLM-based code generation agents. Rather than replacing the LLM, Marky serves as a validation and guidance layer, helping agents generate more idiomatic code in real-time.

**How Agents Use Marky for Guidance**

The integration pattern is straightforward: instead of a simple generate-then-review cycle, Marky enables a continuous feedback loop during generation.
...
↓
After each statement, Marky analyzes:
...
Here's how a generation might proceed with Marky validation:
...
ITERATION 1:
  ...
  Marky analysis: guard-clause detected
...
ITERATION 2:
  ...
  Marky analysis: return-bool confidence 0.8 (common after guards)
  Marky analysis: return-none confidence 0.15 (less common)
  Action: Continue, both valid patterns

ITERATION 3:
  Generated: "    email_regex = r'...'"
  Pattern: init-var (assignment)
  Marky analysis: After return-bool, return-var confidence 0.3
  Marky analysis: After return-bool, init-var confidence 0.15
...
ITERATION 4:
  ...
  Marky analysis: After init-var, return-computed confidence 0.7
...
Suppose Marky analysis shows: "After a function definition, guards have 0.85 confidence."
...
Marky feedback: loop-try-except pattern (confidence: 0.2)
...
Marky feedback: AST error detected - code after return
...
```
As code generates, Marky tracks:
...
```
Agent generates code → Marky validates → Report results
Simplest approach, no feedback loop.
```

*Pattern 2: Temperature-Aware*
```
Agent generates tokens → Marky scores pattern → Adjust temperature
...
User reviews with Marky feedback
...
Agent B (validator) uses Marky to score
...
class MarkyGuidedAgent:
    """
    Example of Marky integration with an LLM agent.
    """
    ...
    def generate_with_guidance(
        ...
        """
        Generate code with Marky guidance and iterative refinement.
    ...
    def _adjust_temperature(self, confidence: float) -> float:
        """Dynamically adjust temperature based on Marky confidence."""
        if confidence > 0.8:
...
Combining Marky with LLM agents offers several advantages:
...
Training speed is crucial for iterative development. Marky processes code quickly:
...
Validation speed determines whether Marky can be used in real-time workflows:
...
```
Marky training: 5.6s for 596 files
Pylint: ~60s for 596 files (10x slower)
Flake8: ~15s for 596 files (3x slower)

Marky validation: ~200ms per file
Pylint: ~300ms per file
Flake8: ~100ms per file

Advantage: Marky learns from your code, not fixed rules
Trade-off: Marky requires initial training step
```
...
Advantage: Marky checks patterns, not types (complementary)
...
Marky inference: <1ms per lookup
CodeBERT inference: ~100ms per prediction
GPT inference: ~1s per completion

Advantage: Marky is 100-1000x faster
Trade-off: Marky doesn't understand semantics, only patterns
```
...
- **Tool version**: Marky current git HEAD
- **Method**:
...
Marky's pattern-learning approach enables a variety of practical applications, from code quality enforcement to training data analysis. Here are detailed scenarios showing how Marky solves real problems.
...
Marky learns what "quality code" looks like from your existing codebase, then validates new code against those patterns.

*Example Workflow*:

```bash
# 1. Train on your production codebase
uv run python -m src train src/ models/ --model-type both

# 2. Validate new code during PR review
uv run python -m src validate models/semantic_model.py new_feature.py
...
A team maintaining a Flask application trains Marky on their existing routes, models, and utilities:
...
Marky identifies the dominant patterns in your codebase and flags deviations. This creates a "style fingerprint" unique to your project.

*Example Workflow*:

```bash
# Train on well-styled reference code
uv run python -m src train examples/good_style/ models/

# Validate new contributions
uv run python -m src validate models/semantic_model.py contribution.py

# Integrate into pre-commit hook
# .git/hooks/pre-commit:
#!/bin/bash
for file in $(git diff --cached --name-only --diff-filter=ACM | grep '\.py$'); do
    uv run python -m src validate models/semantic_model.py "$file"
    if [ $? -ne 0 ]; then
...
Marky extracts and quantifies patterns, giving you insights into code characteristics.

*Example Workflow*:

```bash
# Train model
uv run python -m src train codebase/ analysis_models/

# View statistics
uv run python -m src stats analysis_models/semantic_model.py
...
uv run python -m src train src/api/ models/api_model.py
uv run python -m src train src/db/ models/db_model.py
uv run python -m src train src/utils/ models/utils_model.py
...
Train Marky on your codebase, then use it to guide or validate LLM-generated code.
...
uv run python -m src train src/ models/ --exclude tests/

# Validate suspicious file
uv run python -m src validate models/semantic_model.py suspicious.py
...
A bug report comes in for a function. Marky validation shows:
...
The bug: `update_balance` is never called because of early return. Marky flagged it as unusual because similar functions in the codebase don't have this pattern.
...
for file in src/**/*.py; do
    uv run python -m src validate models/semantic_model.py "$file" >> results.txt
done
...
Integrate Marky into your development workflow to validate code automatically.
...
for file in $(git diff --cached --name-only --diff-filter=ACM | grep '\.py$'); do
    result=$(uv run python -m src validate models/semantic_model.py "$file" 2>&1)
...
echo "All files passed Marky validation!"
```
      ...
      - name: Train model (or use cached)
        run: |
          if [ ! -f models/semantic_model.py ]; then
            uv run python -m src train src/ models/
          fi
...
uv run python -m src train old_src/ models/old_style.py

# Train on target examples (new style)
uv run python -m src train examples/new_style/ models/new_style.py

# During migration, validate against new style
uv run python -m src validate models/new_style.py refactored_file.py
```
...
Marky trained on new style flags incomplete migrations:
...
Use Marky statistics to show "how we write code here" with concrete examples.

*Example Workflow*:

```bash
# Generate onboarding report
uv run python -m src stats models/semantic_model.py > ONBOARDING.md
...
uv run python -m src train v1.0/ models/v1.py
uv run python -m src train v2.0/ models/v2.py
uv run python -m src train v3.0/ models/v3.py
...
Marky shows that small, interpretable probabilistic models add high value to code workflows. By learning patterns from your own codebase, Marky provides fast, explainable guidance that complements existing tools rather than replacing them.
...
If you care about consistency, developer productivity, and explainability, try training Marky on a representative subset of your code and see what it highlights.
